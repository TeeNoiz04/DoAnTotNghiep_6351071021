...
export abstract class AbstractSaleReportR06ViewService {
  ...
  data: PagedResultDto<SaleReportByCustomerDto> = {
    items: [],
    totalCount: 0,
  };
  ...
  loadData(filters: SaleReportInput) {
    ...
    this.proxyService
      .getListSaleReportDetail(this.filters)
      .pipe(finalize(() => this.loadingService.hide()))
      .subscribe({
        next: (result: PagedResultDto<SaleReportByCustomerDto>) => {
          this.data = result;
        },
        error: error => {
          console.error('Error loading Sale Report R06:', error);
  ...
  exportToExcel(filters: SaleReportInput): void {
    ...
    this.proxyService
      .getListSaleReportDetailAsExcel(exportFilters)
      ...
      .subscribe({
        next: (blob: Blob) => {
          ...
          this.abpWindowService.downloadBlob(blob, `R06-SaleReport-${dd}${mm}${yy}.xlsx`);
