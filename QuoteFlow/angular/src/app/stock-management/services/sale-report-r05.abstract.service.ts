...
export abstract class AbstractSaleReportR05ViewService {
  ...
  data: PagedResultDto<SaleReportByCustomerR05Dto> = {
    items: [],
    totalCount: 0,
  };

  filters: SaleReportInput = {
    fromDate: null,
    toDate: null,
  ...
  loadData(filters: SaleReportInput) {
    this.loadingService.show();
    this.filters = { ...this.filters, ...filters };

    this.proxyService
      .getListSaleReportGeneral(this.filters)
      .pipe(finalize(() => this.loadingService.hide()))
      .subscribe({
        next: (result: PagedResultDto<SaleReportByCustomerR05Dto>) => {
          this.data = result;
        },
        error: error => {
          console.error('Error loading Sale Report R05:', error);
          this.toast.error('Failed to load report data. Please try again.');
        },
      });
  }

  exportToExcel(filters: SaleReportInput): void {
    ...
    this.proxyService
      .getListSaleReportGeneralAsExcel(exportFilters)
      ...
      .subscribe({
        next: (blob: Blob) => {
          ...
          this.abpWindowService.downloadBlob(blob, `R05-SaleReport-${dd}${mm}${yy}.xlsx`);
