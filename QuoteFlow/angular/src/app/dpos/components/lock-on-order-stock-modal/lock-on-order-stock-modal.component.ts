...
@Component({
  selector: 'app-lock-on-order-stock-modal',
  standalone: true,
  imports: [CommonModule, FormsModule, ThemeSharedModule, NgSelectModule, ReactiveFormsModule, PermissionDirective],
  templateUrl: './lock-on-order-stock-modal.component.html',
  styleUrls: ['./lock-on-order-stock-modal.component.scss'],
})
export class LockOnOrderStockModalComponent implements OnInit {
  ...
  onLockOnOrderStock(): void {
    ...
    if (!this.selectedItems || this.selectedItems.length === 0) {
      this.toasterService.warn('No items selected for lock on order stock', 'Validation Error');
      return;
    }
  ...
  onReleaseOnOrderStock(): void {
    if (!this.selectedItems || this.selectedItems.length === 0) {
      this.toasterService.warn('No items selected for release on order stock', 'Validation Error');
      return;
    }
    ...
    this.extendedService
      .batchAutoUnlockOnOrderStockWithProgress(request)
