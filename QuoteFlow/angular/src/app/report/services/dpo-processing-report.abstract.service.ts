...
export abstract class AbstractDPOProcessingReportViewService {
  ...
  data: PagedResultDto<DPOProcessingReportDto> = {
    items: [],
    totalCount: 0,
  };
  ...
  //             }) as PagedResultDto<DPOProcessingReportDto>,
  //         ),
  //       );

  //   const setData = (list: PagedResultDto<DPOProcessingReportDto>) => {
  //     this.data = list;
  //   };

  //   // Hủy subscription cũ để tránh memory leak
  ...
  //   // ✅ Fetch data ngay lần đầu (không cần chờ trigger từ list)
  //   getData({ skipCount: 0, maxResultCount: 1000 }).subscribe({
  ...
  exportToExcel(filters: GetDPOReportInputDto): void {
    ...
    this.proxyService
      .getListDPOProcessingReport(filters)
      .pipe(finalize(() => (this.isExportToExcelBusy = false)))
      .subscribe({
        next: (blob: Blob) => {
          const now = new Date();
          const dateStr = now.toISOString().split('T')[0];
          this.abpWindowService.downloadBlob(blob, `DPO_Processing_Report_${dateStr}.xlsx`);
