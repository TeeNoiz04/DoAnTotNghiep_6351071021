...
import { GetPriceOfferMessagesInput } from '../../../../../proxy/price-offers/price-offer-messages/models';
...
@Component({
  ...
  styleUrls: ['./discussion-box.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class DiscussionBoxComponent implements OnInit, OnChanges, OnDestroy, AfterViewChecked {
  @Input() isOpen = false;
  @Input() priceOfferId: string;
  @Input() priceOfferCode: string;
  ...
  currentPage = 0;
  messagesPerLoad = 20;
  toRecipientCollapseLength = 40; // Length to collapse recipient list
  emailChips: string[] = [];
  ...
  ngOnInit(): void {
    this.initForm();
    // Load messages immediately if discussion box is open and priceOfferId is available
    if (this.isOpen && this.priceOfferId) {
      this.loadMessages();
    }
  }
  ...
  ngOnChanges(changes: any): void {
    ...
    if (this.priceOfferId && this.messages.length === 0 && !this.isLoadingMessages) {
      this.loadMessages();
    }
  ...
  sendMessage(): void {
    if (this.messageForm.invalid || !this.priceOfferId) return;
    ...
    this.priceOfferService
      .sendMessage(this.priceOfferId, messageInput)
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => {
