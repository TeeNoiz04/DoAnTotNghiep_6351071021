...
@Component({
  selector: 'app-result-import-table',
  templateUrl: './result-import-table.component.html',
  styleUrls: ['./result-import-table.component.scss'],
  standalone: true,
  imports: [CommonModule, StatusLabelComponent, HistoryModalComponent],
})
export class ResultImportTableComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  @Input() data: any[] = [];
  ...
  @Input() numberColumnFixed: number = 3;
  ...
  getCellClass(column: TableColumn, index?: number, row?: any): string {
    ...
    if (column.fixed || (index !== undefined && index < this.numberColumnFixed)) {
      classes += ' fixed-column';
    }

    // Add custom cell class if provided
    if (column.cellClass) {
      // Cell class logic is handled in the next conditional block
    }
  ...
  getHeaderClass(column: TableColumn, index: number): string {
    ...
    // Add fixed column class for the first 3 columns
    if (column.fixed || index < this.numberColumnFixed) {
      classes += ' fixed-column';
    }
