...
public class SaleOrderGICFOCExcelValidator : BaseExcelValidator<SaleOrderGICFOCExcelDto>
{
    ...
    public SaleOrderGICFOCExcelValidator(ExcelValidationConfig config, IExcelRowValidator<SaleOrderGICFOCExcelDto> rowValidator, ILogger<BaseExcelValidator<SaleOrderGICFOCExcelDto>> logger, IServiceProvider provider) : base(config, rowValidator, logger)
    {
        _provider = provider;
    ...
    protected override async Task PostValidateAsync(ExcelValidationResult<SaleOrderGICFOCExcelDto> result)
    {
        ...
        foreach (var row in result.ListData)
        {
            ...
            if (!exitsMaterial)
            {
                ...
                result.Errors.Add($"Material code '{row.RowData.MaterialCode}' with model '{row.RowData.ModelName}' is not exist.");
                continue;
            }
            ...
            {
                var materialExistsInSO = saleOrderDetail.Any(x =>
                    x.SOId == so.Id &&
                    x.MaterialCode == row.RowData.MaterialCode);

                if (!materialExistsInSO)
                    row.Errors.Add($"Material '{row.RowData.MaterialCode}' with model '{row.RowData.ModelName}' was not found in the SO No '{row.RowData.SONo}'.");

                var existSONo = saleOrder.FirstOrDefault(x =>
                    x.SONo == row.RowData.SONo &&
                    (x.Status == QuoteFlowStatuses.InProgress || x.Status == QuoteFlowStatuses.Closed));
            ...
            // Add all collected row errors to result
            if (row.HasErrors)
    ...
    private static void ValidateRequiredFieldsToClose(ExcelRowResult<SaleOrderGICFOCExcelDto> row)
    {
        if (string.IsNullOrWhiteSpace(row.RowData.SAPSONo))
            row.Errors.Add($"SAP SO No (W) is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.DONo))
            row.Errors.Add($"SAP DO No (X)  is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.BillingNo))
            row.Errors.Add($"SAP Billing No (Y) is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.GICGivNo))
            row.Errors.Add($"GIV No (Z) is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.InvoiceNo))
            row.Errors.Add($"Invoice No (AA) is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.InvoiceDate?.ToString()))
            row.Errors.Add($"Invoice Date (AB) is required or invalid.");

        //if (string.IsNullOrWhiteSpace(row.RowData.GICSAPLandingCost?.ToString()))
        //    row.Errors.Add($"SAP Landing Cost (AC) is required or invalid.");

        //if (string.IsNullOrWhiteSpace(row.RowData.GICPORNo))
        //    row.Errors.Add($"POR No (AE) is required.");
