...
public class SaleOrderGICWriteOffExcelValidator : BaseExcelValidator<SaleOrderGICWriteOffExcelDto>
{
    ...
    public SaleOrderGICWriteOffExcelValidator(ExcelValidationConfig config, IExcelRowValidator<SaleOrderGICWriteOffExcelDto> rowValidator, ILogger<BaseExcelValidator<SaleOrderGICWriteOffExcelDto>> logger, IServiceProvider provider) : base(config, rowValidator, logger)
    {
        _provider = provider;
    ...
    protected override async Task PostValidateAsync(ExcelValidationResult<SaleOrderGICWriteOffExcelDto> result)
    {
    ...
    private static void ValidateRequiredFieldsToClose(ExcelRowResult<SaleOrderGICWriteOffExcelDto> row)
    {
        if (string.IsNullOrWhiteSpace(row.RowData.SONo))
            row.Errors.Add($"SO No (P) is required.");

        if (string.IsNullOrWhiteSpace(row.RowData.GIVDate?.ToString()))
            row.Errors.Add($"GIV Date (Q) is required or invalid.");

        if (string.IsNullOrWhiteSpace(row.RowData.SAPLandingCost?.ToString()))
            row.Errors.Add($"SAP Landing Cost (R) is required or invalid.");


        var cost = row.RowData.SAPLandingCost;
        var quantity = row.RowData.SOQty;
        var amount = row.RowData.AmountInSAPLandingCost;
