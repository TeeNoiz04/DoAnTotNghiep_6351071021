...
public class PriceOfferAPDetailRowValidator : IExcelRowValidator<PriceOfferDetailImportDto>
{
    // we can do dependency injection here if needed, e.g. for logging or other services
    public PriceOfferAPDetailRowValidator()
    {
    ...
    public ValidationResult ValidateRow(IDictionary<string, object> rowData, int rowIndex)
    {
        ...
        if (standardPrice.HasValue && !decimal.IsInteger(standardPrice.Value)) result.AddError("Standard Price must be a whole number.");
        if (buyerPrice.HasValue && !decimal.IsInteger(buyerPrice.Value)) result.AddError("Buyer Price must be a whole number.");
        if (MEVNOfferPrice.HasValue && !decimal.IsInteger(MEVNOfferPrice.Value)) result.AddError("MEVN Offer Price must be a whole number.");
    ...
    public PriceOfferDetailImportDto ParseRow(IDictionary<string, object> rowData)
    {
        ...
        var MEVNOfferPrice = ExcelParser.GetValue<decimal?>(rowData, "M");
        var competitorBrand = ExcelParser.GetValue<string>(rowData, "N");
        var competitorModel = ExcelParser.GetValue<string>(rowData, "O");
        var competitorPrice = ExcelParser.GetValue<decimal?>(rowData, "P");
