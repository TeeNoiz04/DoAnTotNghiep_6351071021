...
public class PriceOfferAddMoreItemRowValidator : IExcelRowValidator<PriceOfferDetailImportDto>
{
    // we can do dependency injection here if needed, e.g. for logging or other services
    public PriceOfferAddMoreItemRowValidator()
    {
    ...
    public ValidationResult ValidateRow(IDictionary<string, object> rowData, int rowIndex)
    {
        ...
        if (qty is null || qty == 0) result.AddError("Qty must not be 0.");
        ...
        //var (IsValid, _, ErrorMessage) = ExcelValidationHelper.ValidateAmountCalculation(qty, buyerPrice, requestedAmountProvided, "Requested Amount");
    ...
    public PriceOfferDetailImportDto ParseRow(IDictionary<string, object> rowData)
    {
        ...
        // Calculate amounts and discount ratio
        ...
        // Calculate discount ratio: (StandardPrice - BuyerPrice) / StandardPrice * 100
