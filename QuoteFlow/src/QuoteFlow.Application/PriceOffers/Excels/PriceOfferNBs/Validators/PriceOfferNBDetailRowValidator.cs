...
public class PriceOfferNBDetailRowValidator : IExcelRowValidator<PriceOfferDetailImportDto>
{
    // we can do dependency injection here if needed, e.g. for logging or other services
    public PriceOfferNBDetailRowValidator()
    {
    ...
    public ValidationResult ValidateRow(IDictionary<string, object> rowData, int rowIndex)
    {
        ...
        if (buyerPrice is not null && buyerPrice < 0) result.AddError("Buyer Price must be greater than 0.");
